...
```{r}
subset2<- BD_MUJERES_ENDIREH2016_sample[, c("sexo", "edad", "niv")]
```

podemos combinar los dos tipos de selección
```{r}
subset3<- BD_MUJERES_ENDIREH2016_sample[(BD_MUJERES_ENDIREH2016_sample$niv>4 & BD_MUJERES_ENDIREH2016_sample$sexo==2 ), c("sexo", "edad", "niv")]
```
...
```{r}
BD_MUJERES_ENDIREH2016_sample %>%
  tabyl(sexo) %>% # para hacer la tabla
...
```{r}
sexo.lab<-c("Hombre", "Mujer")

BD_MUJERES_ENDIREH2016_sample$sexo<-set_labels(BD_MUJERES_ENDIREH2016_sample$sexo, labels=sexo.lab) # establece las etiquetas
```
...
BD_MUJERES_ENDIREH2016_sample %>% 
  ...
  mutate(sexo=as_label(sexo)) %>% # para que las lea como factor
                tabyl(niv, sexo, show_missing_levels=F ) %>% # incluimos aquí 
                adorn_totals()  
```
...
```{r}
BD_MUJERES_ENDIREH2016_sample %>%   
    count(niv==1 & sexo==1) # nos da la segunda celda de la izquierda
...
BD_MUJERES_ENDIREH2016_sample %>% 
  mutate(niv=as_label(niv)) %>% # para que las lea como factor
  mutate(sexo=as_label(sexo)) %>% # para que las lea como factor
    tabyl(niv, sexo, show_missing_levels=F ) %>% # incluimos aquí sexoo
                adorn_totals("col")  
```
...
BD_MUJERES_ENDIREH2016_sample %>% 
  mutate(niv=as_label(niv)) %>% # para que las lea como factor
  mutate(sexo=as_label(sexo)) %>% # para que las lea como factor
  tabyl(niv, sexo, show_missing_levels=F ) %>% # incluimos aquí sexo
                adorn_totals(c("col", "row")) %>% 
...
BD_MUJERES_ENDIREH2016_sample %>% 
  mutate(niv=as_label(niv)) %>% # para que las lea como factor
  mutate(sexo=as_label(sexo)) %>% # para que las lea como factor
                tabyl(niv, sexo, show_missing_levels=F ) %>% 
                adorn_totals(c("col", "row")) %>% 
...
BD_MUJERES_ENDIREH2016_sample %>% 
 group_by(as_label(sexo)) %>% 
  tally(fac_viv) %>% #nombre del factor
...
```{r}
BD_MUJERES_ENDIREH2016_sample %>% 
  count(as_label(sexo), as_label(niv),  wt = fac_viv) 
```
...
```{r}
BD_MUJERES_ENDIREH2016_sample %>% 
  mutate_at(vars(sexo, niv), as_label) %>% 
  count(sexo, niv,  wt = fac_viv) %>% 
...
```{r}
BD_MUJERES_ENDIREH2016_sample %>% 
  mutate_at(vars(sexo, niv), as_label) %>% # otra forma de mutate y as_label
  count(sexo, niv,  wt = fac_viv) %>% 
  pivot_wider(names_from = sexo, 
              values_from = n) %>%
   adorn_totals() %>% # Agrega total
